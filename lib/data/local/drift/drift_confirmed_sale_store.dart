import 'drift_customer_credit_store.dart';
import 'dart:convert';
...
class DriftConfirmedSaleStore implements ConfirmedSaleStore {
  ...
  Future<void> apply(
    ...
    if (p.clienteId != null) {
      ...
      if (cliente == null || cliente.createdEventId != p.clienteEventId) {
        throw StateError('Falta el cliente de la venta.');
      }
    }
    ...
    await (db.update(db.sales)..where((t) => t.id.equals(event.aggregateId)))
        .write(SalesCompanion(clienteId: Value(p.clienteId)));
    if (p.isCredit) {
      await db
          .into(db.creditSales)
          .insert(
      ...
      await DriftCustomerCreditStore(db).rebuild(p.clienteId!);
    } else {
      await db
      ...
      for (final c in line.consumptions) {
        ...
        final balance =
            await (db.select(db.inventoryBalances)
                  ..where((b) => b.inventoryItemId.equals(c.inventoryItemId)))
                .getSingle();
        int next(int n) {
  ...
  Future<void> acknowledge(String eventId, int serverSequence) =>
      db.transaction(() async {
        await DriftCustomerCreditStore(db).acknowledge(eventId, serverSequence);
