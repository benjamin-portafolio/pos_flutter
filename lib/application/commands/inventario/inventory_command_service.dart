...
class InventoryCommandService {
  ...
  Future<bool> editarRecurso(EditarRecursoInventarioCommand command) async {
    ...
    final name = NombreRecursoInventario.fromInput(command.nombre).value;
    final reason = _validateMovementFields(command);
    ...
    if (name != item.name) {
      final renamed = _buildRenameEvent(item, baseEventId, name);
      entries.add(renamed);
      nextBaseEventId = renamed.event.eventId;
      nextBaseVersion += 1;
  ...
  String? _validateMovementFields(EditarRecursoInventarioCommand command) {
  ...
  LocalEventAppend _buildRenameEvent(
    ...
    return LocalEventAppend(
      ...
      refs: [LocalEventRef.affects(refType: 'inventory_item', refId: item.id)],
    );
  }
